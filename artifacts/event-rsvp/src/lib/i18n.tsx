...
const EN = {
  ...
  confirmWarnTitle: 'Important: your RSVP is not sent yet',
  confirmWarn: 'You must press the submit button below to finish. If you leave without pressing it, your RSVP will not be submitted.',
  ...
  errChildBlank: 'Enter this child’s name and group, or remove the row.',
  errChildName: (n: number) => `Please enter child ${n}’s name.`,
  errChildGroup: (name: string) => `Please choose ${name}’s group.`,
  ...
  none: 'None',
  // A saved child: the name, then its group (or the age older RSVPs were saved with).
  childWith: (name: string, detail: string) => (detail ? `${name} (${detail})` : name),
  years: (age: number) => `age ${age}`,
...
const KO: typeof EN = {
  ...
  confirmWarnTitle: '중요: 아직 제출되지 않았습니다',
  confirmWarn: '아래 「RSVP 제출하기」 버튼을 꼭 클릭하셔야 등록이 완료됩니다. 버튼을 누르지 않고 페이지를 나가시면 RSVP가 제출되지 않습니다.',
  submitting: '등록 중…',
  submit: 'RSVP 제출하기',
  editAgain: '다시 수정하기',
  ...
  errChildBlank: '아이 이름과 그룹을 입력하거나 이 칸을 삭제해 주세요.',
  errChildName: (n: number) => `${n}번째 아이의 이름을 입력해 주세요.`,
  errChildGroup: (name: string) => `${name}의 그룹을 선택해 주세요.`,
